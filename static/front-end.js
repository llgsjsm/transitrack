document.addEventListener('DOMContentLoaded', function () {
    ...
    //button to simulate the breakdown
    const submitButton2 = document.getElementById('submitButton2');
    if (submitButton2) {
        submitButton2.addEventListener('click', function (event) {
            event.preventDefault();
            const breakdown = document.getElementById('breakdown_select').value.trim().toLowerCase();
            if (breakdown) {
                fetch(`/api/breakdown/?station=${breakdown}`)
                    .then(response => {
                        if (!response.ok) {
                    ...
                    .then(data => {
                        if (data.route !== 'null') {
                            ...
                            data.route.forEach((station, index) => {
                                ...
                                if (data.duration > 998) {
                                    resultTime.textContent = stationInfo ? stationInfo.duration + ' min' : `Breakdown at ${breakdown}`;
                                }
                            ...
                            //display alternative suttle bus
                            if (data.duration > 998) {
                                analysisTotalTime.textContent = `MRT breakdown alerted at ${(breakdown)}.\n`;
                                if (breakdown == fromBoxValue) {
                                    analysisTotalTime.textContent += `Alternative shuttle buses available at ${(data.results)}.`;
                                } else if (breakdown == toBoxValue) {
                                    analysisTotalTime.textContent += `Alternative shuttle buses available at ${(data.results)}.`;
                                } else {
    ...
    //for textbox, perform search function. From: sequential search | To: Binary search 
    ...
    function markPath(route, breakdown) {
        if (!circles.length) {
        ...
        circles.forEach(circle => {
            ...
            if (stationName && route.includes(stationName)) {
                ...
                //if breakdown station then highlight red
                if (stationName.toLowerCase() == breakdown) {
    ...
    function getMostOptimalAlgorithm(fromBoxValue, toBoxValue) {
        ...
        algoList.forEach(algo => {
            ...
            if (request.status === 200) {
                ...
                //compare the duration of the journey first, then if same, compare the time execution the algorithm took
