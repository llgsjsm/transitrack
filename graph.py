...
# Helper function to simulate the breakdown (POC)
def graph_update(file_name, bd_index, path, breakdown):
    # Open and read the bus.json file
    with open('static/bus.json', 'r') as file:
        bus_data = json.load(file)

    for i in path:
        if i == breakdown:
            bd_index = path.index(i)
            break
    ...
    if bd_index != -1:
        ...
        #IF breakdown is in the middle, return buses from breakdown-1 and breakdown+1
        else:
            for edge in bus_data["bus"]:
                if (edge["from"].lower() == path[bd_index - 1] and edge["to"].lower() == path[bd_index + 1]):
                    return edge["from"], edge["to"]

# Helper function to load the graph from a JSON file
def load_graph(file_name, breakdown):
    ...
    for edge in data["routes"]:
        ...
        graph[from_station].append({"to": to_station, "duration": duration, "distance": edge["distance"]})
        graph[to_station].append({"to": from_station, "duration": duration, "distance": edge["distance"]})
...
#Depth first search: LIN ZHENGXU 
#choosen to use iterative DFS over recursive due to lesser overhead and faster execution time
def dfs(graph, start, end):
    start_time = time.perf_counter()  # Start time
    path = []
    stack = [(start, 0, [])]  # (current_node, current_duration, current_path)
    visited = set()

    while stack:
        node, current_duration, current_path = stack.pop()
        if node in visited:
            continue
        ...
        for next_station in graph[node]:
            if next_station["to"] not in visited:
                stack.append((next_station["to"], current_duration + next_station["duration"], current_path))
